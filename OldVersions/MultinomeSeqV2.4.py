...
class SequencerGUI:
    def __init__(self,root,be:Backend):
        ...
        # row2 clock + midi + chan
        mid=tk.Frame(ctrl,bg="#222"); mid.grid(row=2,column=0,columnspan=5,sticky="w",pady=4)
        ...
        self.port_menu = tk.OptionMenu(mid, self.port, self.port.get(), *outs, command=self.be.set_port)
        self.port_menu.pack(side="left", padx=4)
        tk.Button(mid, text="Refresh", font=BF, command=self._refresh_midi_ports).pack(side="left", padx=4)
        ...
        self.track_name_entry = tk.Entry(ctrl, textvariable=self.track_name_var, font=LF,
                                         bg="#555", fg="#ddd", width=10, justify='center',
                                         bd=0, highlightthickness=1, highlightbackground="#444")
        ...
        tk.Checkbutton(ctrl,text="Mute",variable=self.mute_var,selectcolor="#222",
                       command=self._toggle_mute,bg="#222",fg="#ddd")\
